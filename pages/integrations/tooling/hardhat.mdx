...
const config: HardhatUserConfig = {
  networks: {
    // For Superseed Mainnet
    "superseed-mainnet": {
      url: "https://mainnet.superseed.org",
      accounts: [process.env.WALLET_KEY as string],
      gasPrice: 1000000000,
    },
    // For Superseed Sepolia Testnet
    "superseed-sepolia": {
      url: "https://sepolia.superseed.org",
...
## Your Smart Contracts on Superseed Mainnet
...
```bash
npx hardhat run scripts/deploy.ts --network superseed-mainnet
```
...
```bash
npx hardhat verify --network superseed-mainnet DEPLOYED_CONTRACT_ADDRESS "Constructor argument 1"
```
...
<Callout type="info" emoji="ℹ️">
  For deploying on the Superseed Testnet, just replace the `—-network` parameter
